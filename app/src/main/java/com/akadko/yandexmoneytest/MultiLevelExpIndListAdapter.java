...
/**
 * Created by akadko on 01.08.2015.
 */
...
public abstract class MultiLevelExpIndListAdapter extends RecyclerView.Adapter {
    ...
    public void addAll(int position, Collection<? extends ExpIndData> data) {
        if (data != null && data.size() > 0) {
            ...
            for (ExpIndData item : data) {
                if (item.getChildren() != null && !item.getChildren().isEmpty()) {
                    mGroups.put(item, item.getChildren());
                    for (ExpIndData child : item.getChildren())
                        if (child.getChildren() != null && !child.getChildren().isEmpty())
                        mGroups.put(child, child.getChildren());
                }
            }
        }
    }
